...
QWEN["千问服务(QwenService)<br/>DashScope"]
...
- 外部集成：DashScope千问提供语言模型能力；MinIO/Etcd按需扩展
...
- 关键流程
  ...
  - RAG对话：生成问题向量 → Milvus检索 → 千问生成回答 → 持久化对话
...
participant Q as "QwenService(DashScope)"
...
### 组件D：千问服务与嵌入
- 职责
  - 调用DashScope千问模型进行对话与RAG增强
  - 生成问题向量（当前实现为占位，实际项目可替换为专用embedding服务）
- 配置
  - 通过环境变量设置API Key与模型名称
  - 系统提示词定制，强调教务场景与安全约束
...
## 依赖关系分析
- 技术栈与依赖
  - 后端：FastAPI、Requests、BeautifulSoup、Selenium/Pyppeteer、Redis、Milvus、LangChain生态、OpenAI/DashScope
...
## 结论
本系统通过Next.js + FastAPI + LangChain生态（DashScope千问）实现了教务数据查询与RAG增强对话的完整闭环。容器化编排清晰分离了前端、后端与数据/中间件服务，具备良好的可扩展性与可维护性。后续可在嵌入模型、索引参数与缓存策略上持续优化，以提升检索质量与系统吞吐。
