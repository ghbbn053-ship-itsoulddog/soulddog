# 第三方服务集成

<cite>
**本文引用的文件**
- [backend/main.py](file://backend/main.py)
- [backend/scraper.py](file://backend/scraper.py)
- [backend/app/api/education.py](file://backend/app/api/education.py)
- [backend/app/api/mcp.py](file://backend/app/api/mcp.py)
- [backend/app/api/agents.py](file://backend/app/api/agents.py)
- [backend/app/api/models.py](file://backend/app/api/models.py)
- [backend/app/mcp/tools.py](file://backend/app/mcp/tools.py)
- [backend/mcp_server.py](file://backend/mcp_server.py)
- [backend/app/services/agent_runtime.py](file://backend/app/services/agent_runtime.py)
- [backend/app/services/model_provider.py](file://backend/app/services/model_provider.py)
- [backend/app/services/qwen_service.py](file://backend/app/services/qwen_service.py)
- [openclaw/skill.json](file://openclaw/skill.json)
- [backend/app/services/vector_store.py](file://backend/app/services/vector_store.py)
- [backend/docker-compose.yml](file://backend/docker-compose.yml)
- [backend/Dockerfile](file://backend/Dockerfile)
- [backend/requirements.txt](file://backend/requirements.txt)
- [backend/test_login.py](file://backend/test_login.py)
- [backend/test_scraper.py](file://backend/test_scraper.py)
- [frontend/Dockerfile](file://frontend/Dockerfile)
- [package.json](file://package.json)
- [scripts/build.sh](file://scripts/build.sh)
- [scripts/start.sh](file://scripts/start.sh)
- [README-Windows.md](file://README-Windows.md)
- [README.md](file://README.md)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向"智能教务系统"的第三方服务集成扩展，围绕以下主题提供系统化指导：
- 如何接入新的外部服务与API（认证、数据同步、错误处理）
- 服务发现与配置管理（环境变量、服务注册、负载均衡）
- 缓存服务集成（Redis集群、缓存策略、数据一致性）
- 消息队列与异步任务（任务调度、失败重试、监控告警）
- 日志与监控（分布式追踪、指标采集、告警通知）
- 安全服务集成（OAuth、API密钥、访问控制）
- DevOps工具链（CI/CD、容器编排、自动化部署）
- **新增** AI代理标准化交互协议（MCP集成，OpenClaw支持）
- **新增** LangChain、LangGraph、OpenAI等AI框架集成
- **新增** 统一模型提供层（Model Provider）架构

本项目已具备基础的后端API、爬虫模块、向量数据库（Milvus）与缓存（Redis）能力，并新增了完整的MCP（Model Context Protocol）集成、AI代理运行时服务以及统一的模型提供层，可作为扩展第三方服务与AI代理的标准接口。

## 项目结构
后端采用FastAPI，提供教务系统数据爬取与聚合接口；前端基于Next.js；通过Docker Compose编排数据库、缓存、对象存储与向量数据库等依赖服务。新增的MCP模块提供了标准化的AI工具调用接口，AI代理运行时服务支持多种AI框架集成。

```mermaid
graph TB
subgraph "前端"
FE_Docker["前端Dockerfile"]
FE_Pkg["package.json"]
end
subgraph "后端"
BE_Main["main.py"]
BE_Scraper["scraper.py"]
BE_Vector["app/services/vector_store.py"]
BE_API["app/api/education.py"]
BE_MCP_API["app/api/mcp.py"]
BE_AGENT_API["app/api/agents.py"]
BE_MODEL_API["app/api/models.py"]
BE_MCP_Server["mcp_server.py"]
BE_MCP_Tools["app/mcp/tools.py"]
BE_AgentRuntime["app/services/agent_runtime.py"]
BE_ModelProvider["app/services/model_provider.py"]
BE_QwenService["app/services/qwen_service.py"]
BE_Docker["backend/Dockerfile"]
BE_Req["requirements.txt"]
end
subgraph "依赖服务"
DC["docker-compose.yml"]
Postgres["Postgres"]
Redis["Redis"]
MinIO["MinIO"]
Milvus["Milvus"]
Etcd["etcd"]
end
subgraph "AI代理集成"
OpenClaw["OpenClaw MCP客户端"]
Claude["Claude Desktop"]
Agent["其他MCP兼容AI代理"]
LangChain["LangChain框架"]
LangGraph["LangGraph编排"]
OpenAI["OpenAI API"]
Qwen["通义千问"]
end
FE_Docker --> BE_Main
FE_Pkg --> FE_Docker
BE_Main --> BE_Scraper
BE_Main --> BE_API
BE_Main --> BE_MCP_API
BE_Main --> BE_AGENT_API
BE_Main --> BE_MODEL_API
BE_MCP_API --> BE_MCP_Tools
BE_MCP_Server --> BE_MCP_Tools
BE_AgentRuntime --> LangChain
BE_AgentRuntime --> LangGraph
BE_AgentRuntime --> OpenAI
BE_ModelProvider --> Qwen
BE_ModelProvider --> OpenAI
BE_ModelProvider --> LiteLLM
BE_Vector --> Milvus
BE_API --> Postgres
BE_Docker --> BE_Main
BE_Req --> BE_Docker
DC --> Postgres
DC --> Redis
DC --> MinIO
DC --> Milvus
DC --> Etcd
OpenClaw --> BE_MCP_API
Claude --> BE_MCP_API
Agent --> BE_MCP_API
```

**图示来源**
- [backend/docker-compose.yml:1-167](file://backend/docker-compose.yml#L1-L167)
- [backend/Dockerfile:1-18](file://backend/Dockerfile#L1-L18)
- [frontend/Dockerfile:1-33](file://frontend/Dockerfile#L1-L33)
- [backend/main.py:1-853](file://backend/main.py#L1-L853)
- [backend/scraper.py:1-1220](file://backend/scraper.py#L1-L1220)
- [backend/app/services/vector_store.py:1-164](file://backend/app/services/vector_store.py#L1-L164)
- [backend/app/api/education.py:1-104](file://backend/app/api/education.py#L1-L104)
- [backend/app/api/mcp.py:1-195](file://backend/app/api/mcp.py#L1-L195)
- [backend/app/api/agents.py:1-40](file://backend/app/api/agents.py#L1-L40)
- [backend/app/api/models.py:1-78](file://backend/app/api/models.py#L1-L78)
- [backend/app/mcp/tools.py:1-310](file://backend/app/mcp/tools.py#L1-L310)
- [backend/mcp_server.py:1-35](file://backend/mcp_server.py#L1-L35)
- [backend/app/services/agent_runtime.py:1-177](file://backend/app/services/agent_runtime.py#L1-L177)
- [backend/app/services/model_provider.py:1-299](file://backend/app/services/model_provider.py#L1-L299)
- [backend/app/services/qwen_service.py:1-604](file://backend/app/services/qwen_service.py#L1-L604)
- [backend/requirements.txt:1-58](file://backend/requirements.txt#L1-L58)

**章节来源**
- [backend/docker-compose.yml:1-167](file://backend/docker-compose.yml#L1-L167)
- [backend/Dockerfile:1-18](file://backend/Dockerfile#L1-L18)
- [frontend/Dockerfile:1-33](file://frontend/Dockerfile#L1-L33)
- [backend/requirements.txt:1-58](file://backend/requirements.txt#L1-L58)

## 核心组件
- FastAPI后端服务：提供验证码、登录、个人信息、成绩、课表、培养方案、考试安排、教师/课程查询等接口；内置健康检查与CORS；**新增** MCP HTTP API路由、AI代理运行时API、模型管理API。
- 教务系统爬虫模块：封装验证码获取、登录、个人信息、学籍卡片、成绩、课表、培养方案、学业进度、考试安排、选课信息等数据抓取逻辑。
- **新增** MCP工具模块：将教务系统功能暴露为标准化MCP工具，支持OpenClaw、Claude Desktop等AI代理调用。
- **新增** MCP服务器：提供stdio模式的MCP服务，支持本地调试和OpenClaw集成。
- **新增** OpenClaw技能配置：完整的技能描述、工具定义和使用示例。
- **新增** AI代理运行时服务：支持OpenAI Agents SDK和LangGraph两种AI框架，提供统一的代理执行接口。
- **新增** 统一模型提供层：抽象不同AI模型提供商的差异，支持Qwen、OpenAI、LiteLLM等多种模型。
- **新增** 模型管理API：提供模型提供商切换、模型偏好设置等功能。
- 向量数据库服务（Milvus）：封装连接、集合创建、文档插入、相似度检索、用户数据清理与连接关闭。
- 依赖服务编排：PostgreSQL、Redis、MinIO、Milvus、etcd，通过docker-compose统一管理。
- 前后端Dockerfile与构建脚本：前端使用Nginx静态托管，后端使用uvicorn运行FastAPI。

**章节来源**
- [backend/main.py:1-853](file://backend/main.py#L1-L853)
- [backend/scraper.py:1-1220](file://backend/scraper.py#L1-L1220)
- [backend/app/mcp/tools.py:1-310](file://backend/app/mcp/tools.py#L1-L310)
- [backend/mcp_server.py:1-35](file://backend/mcp_server.py#L1-L35)
- [openclaw/skill.json:1-107](file://openclaw/skill.json#L1-L107)
- [backend/app/services/agent_runtime.py:1-177](file://backend/app/services/agent_runtime.py#L1-L177)
- [backend/app/services/model_provider.py:1-299](file://backend/app/services/model_provider.py#L1-L299)
- [backend/app/services/qwen_service.py:1-604](file://backend/app/services/qwen_service.py#L1-L604)
- [backend/app/api/agents.py:1-40](file://backend/app/api/agents.py#L1-L40)
- [backend/app/api/models.py:1-78](file://backend/app/api/models.py#L1-L78)
- [backend/app/services/vector_store.py:1-164](file://backend/app/services/vector_store.py#L1-L164)
- [backend/docker-compose.yml:1-167](file://backend/docker-compose.yml#L1-L167)
- [frontend/Dockerfile:1-33](file://frontend/Dockerfile#L1-L33)
- [backend/Dockerfile:1-18](file://backend/Dockerfile#L1-L18)

## 架构总览
系统采用"后端API + 爬虫 + 向量数据库 + 缓存 + 对象存储 + 数据库 + MCP工具层 + AI代理运行时 + 统一模型提供层"的组合架构，支持扩展第三方服务与API，以及AI代理的标准化交互。

```mermaid
graph TB
Client["客户端/前端"]
Agent["AI代理/OpenClaw"]
API["FastAPI后端(main.py)"]
MCP_HTTP["MCP HTTP API"]
MCP_STDIO["MCP stdio Server"]
MCP_Tools["MCP工具定义"]
AgentRuntime["AI代理运行时服务"]
ModelProvider["统一模型提供层"]
Qwen["通义千问"]
OpenAI["OpenAI API"]
LiteLLM["LiteLLM"]
Scraper["JwxtScraper(爬虫)"]
EduAPI["教育系统API(外网/内网)"]
Cache["Redis(缓存)"]
VectorDB["Milvus(向量)"]
DB["Postgres(关系数据)"]
Storage["MinIO(对象存储)"]
Config["etcd(配置/服务发现)"]
Client --> API
Agent --> MCP_HTTP
Agent --> MCP_STDIO
MCP_HTTP --> MCP_Tools
MCP_STDIO --> MCP_Tools
AgentRuntime --> ModelProvider
ModelProvider --> Qwen
ModelProvider --> OpenAI
ModelProvider --> LiteLLM
AgentRuntime --> LangChain
AgentRuntime --> LangGraph
AgentRuntime --> OpenAI
MCP_Tools --> Scraper
Scraper --> EduAPI
API --> Cache
API --> VectorDB
API --> DB
API --> Storage
API --> Config
```

**图示来源**
- [backend/main.py:1-853](file://backend/main.py#L1-L853)
- [backend/scraper.py:1-1220](file://backend/scraper.py#L1-L1220)
- [backend/docker-compose.yml:1-167](file://backend/docker-compose.yml#L1-L167)
- [backend/app/services/vector_store.py:1-164](file://backend/app/services/vector_store.py#L1-L164)
- [backend/app/api/mcp.py:1-195](file://backend/app/api/mcp.py#L1-L195)
- [backend/app/mcp/tools.py:1-310](file://backend/app/mcp/tools.py#L1-L310)
- [backend/mcp_server.py:1-35](file://backend/mcp_server.py#L1-L35)
- [backend/app/services/agent_runtime.py:1-177](file://backend/app/services/agent_runtime.py#L1-L177)
- [backend/app/services/model_provider.py:1-299](file://backend/app/services/model_provider.py#L1-L299)
- [backend/app/services/qwen_service.py:1-604](file://backend/app/services/qwen_service.py#L1-L604)

## 详细组件分析

### 组件A：认证与会话管理
- 验证码与登录流程：后端提供获取验证码与登录接口，使用requests.Session维持会话，验证码与登录绑定同一服务器以避免跨节点问题。
- 会话存储：开发态使用内存字典存储会话，生产建议替换为Redis等持久化缓存。
- CORS与健康检查：默认允许所有来源，生产需限制；提供健康检查端点。
- **新增** MCP会话共享：MCP工具通过全局SESSIONS访问现有登录状态，无需重复认证。

```mermaid
sequenceDiagram
participant C as "客户端"
participant API as "FastAPI后端"
participant S as "JwxtScraper"
participant E as "教务系统"
participant M as "MCP工具"
C->>API : GET /api/captcha?username=...
API->>S : 获取验证码
S->>E : 请求验证码图片
E-->>S : 图片字节流
S-->>API : 返回验证码与session
API-->>C : {image, captcha_session_id}
C->>API : POST /api/login {username,password,code,captcha_session_id}
API->>API : 校验参数与session有效性
API->>S : 登录(携带session)
S->>E : 提交登录表单
E-->>S : 登录响应(成功/失败)
S-->>API : 返回登录结果
API-->>C : {success,message,session_id}
Note over M,S : MCP工具通过SESSIONS共享登录状态
```

**图示来源**
- [backend/main.py:135-328](file://backend/main.py#L135-L328)
- [backend/scraper.py:22-60](file://backend/scraper.py#L22-L60)
- [backend/app/mcp/tools.py:21-42](file://backend/app/mcp/tools.py#L21-L42)

**章节来源**
- [backend/main.py:135-328](file://backend/main.py#L135-L328)
- [backend/scraper.py:22-60](file://backend/scraper.py#L22-L60)
- [backend/app/mcp/tools.py:21-42](file://backend/app/mcp/tools.py#L21-L42)

### 组件B：数据同步与聚合
- 数据接口：提供个人信息、学籍卡片、成绩、课表、培养方案、学业进度、考试安排、教师/课程查询等接口。
- 聚合接口：提供"所有数据"接口，便于一次性向量化与RAG入库。
- 爬虫模块：封装HTML解析与数据抽取，支持多种查询参数与统计信息提取。
- **新增** MCP工具：将上述功能封装为标准化MCP工具，支持AI代理调用。

```mermaid
flowchart TD
Start(["进入数据接口"]) --> CheckAuth["检查登录状态"]
CheckAuth --> |未登录| Return401["返回401未授权"]
CheckAuth --> |已登录| BuildScraper["构造JwxtScraper"]
BuildScraper --> CallAPI["调用对应爬取方法"]
CallAPI --> Parse["解析HTML/提取数据"]
Parse --> Success{"是否成功?"}
Success --> |是| ReturnOK["返回数据与计数"]
Success --> |否| ReturnErr["返回错误信息"]
ReturnOK --> End(["结束"])
ReturnErr --> End
Return401 --> End
```

**图示来源**
- [backend/main.py:332-725](file://backend/main.py#L332-L725)
- [backend/scraper.py:61-800](file://backend/scraper.py#L61-L800)
- [backend/app/mcp/tools.py:44-310](file://backend/app/mcp/tools.py#L44-L310)

**章节来源**
- [backend/main.py:332-725](file://backend/main.py#L332-L725)
- [backend/scraper.py:61-800](file://backend/scraper.py#L61-L800)
- [backend/app/mcp/tools.py:44-310](file://backend/app/mcp/tools.py#L44-L310)

### 组件C：向量数据库集成（Milvus）
- 连接与集合：通过环境变量配置主机、端口与集合名，自动创建集合与索引。
- 文档插入：支持批量插入文本、向量、来源与元数据。
- 检索：按用户维度过滤，执行相似度搜索并返回命中结果。
- 清理：按用户ID删除其所有向量数据。

```mermaid
classDiagram
class VectorStore {
+string host
+string port
+string collection_name
+Collection collection
+_connect()
+create_collection(dim)
+add_documents(user_id, texts, embeddings, sources, metadatas) List[int]
+search(user_id, query_embedding, top_k) List[Dict]
+delete_user_data(user_id)
+close()
}
```

**图示来源**
- [backend/app/services/vector_store.py:14-164](file://backend/app/services/vector_store.py#L14-L164)

**章节来源**
- [backend/app/services/vector_store.py:1-164](file://backend/app/services/vector_store.py#L1-L164)

### 组件D：缓存服务集成（Redis）
- 现状：后端使用内存字典存储验证码与登录会话，适合开发；生产需替换为Redis。
- 建议：将验证码session、登录会话、热点查询结果迁移至Redis，结合过期策略与持久化保障一致性。
- 集群与高可用：可参考docker-compose中的Redis服务，扩展为Redis Sentinel或Redis Cluster以提升可用性。

**章节来源**
- [backend/main.py:73-74](file://backend/main.py#L73-L74)
- [backend/docker-compose.yml:20-34](file://backend/docker-compose.yml#L20-L34)

### 组件E：服务发现与配置管理
- 现状：通过环境变量注入服务地址（Postgres、Redis、Milvus、AI模型等），容器间通过服务名通信。
- 建议：引入etcd作为配置中心，集中管理服务注册与动态配置；在后端通过环境变量或配置中心读取服务地址与参数。

**章节来源**
- [backend/docker-compose.yml:113-127](file://backend/docker-compose.yml#L113-L127)
- [backend/main.py:35-37](file://backend/main.py#L35-L37)

### 组件F：消息队列与异步任务
- 现状：未发现显式的异步任务或消息队列组件。
- 建议：引入RQ或Celery + Redis，将耗时任务（如批量数据爬取、向量化入库、报表生成）异步化；配置失败重试与死信队列；结合监控告警。

**章节来源**
- [backend/requirements.txt:1-58](file://backend/requirements.txt#L1-L58)

### 组件G：日志与监控
- 日志：后端使用标准日志模块，建议接入结构化日志（如JSON）与集中式日志系统（如ELK/Fluentd）。
- 监控：建议集成Prometheus + Grafana，暴露指标端点；结合OpenTelemetry实现分布式追踪。
- 告警：基于阈值规则触发告警，对接企业微信/钉钉/邮件。

**章节来源**
- [backend/main.py:35-37](file://backend/main.py#L35-L37)

### 组件H：安全与访问控制
- OAuth：建议在现有登录流程基础上增加OAuth登录入口，统一用户体系。
- API密钥：为第三方服务调用设置API密钥与权限控制，后端在路由层校验。
- 访问控制：基于角色的访问控制（RBAC），限制敏感接口访问。
- **新增** MCP安全：通过工具参数验证和错误处理保障MCP调用安全。

**章节来源**
- [backend/main.py:41-48](file://backend/main.py#L41-L48)
- [backend/app/api/mcp.py:96-146](file://backend/app/api/mcp.py#L96-L146)

### 组件I：DevOps工具链
- CI/CD：建议使用GitHub Actions/GitLab CI，包含依赖安装、构建、测试与镜像推送。
- 容器编排：现有docker-compose满足本地开发；生产建议使用Kubernetes，配合Helm Chart管理。
- 自动化部署：结合GitOps（如ArgoCD）实现声明式部署与回滚。

**章节来源**
- [backend/docker-compose.yml:1-167](file://backend/docker-compose.yml#L1-L167)
- [scripts/build.sh:1-18](file://scripts/build.sh#L1-L18)
- [scripts/start.sh:1-18](file://scripts/start.sh#L1-L18)

### 组件J：AI代理标准化交互（新增）
- **MCP HTTP API**：提供RESTful接口调用MCP工具，支持Web端和其他客户端。
- **MCP工具定义**：将教务系统功能封装为标准化工具，包括成绩查询、课表查询、学业进度等。
- **MCP服务器**：支持stdio模式，便于本地调试和OpenClaw集成。
- **OpenClaw技能配置**：完整的技能描述、工具映射和使用示例。

```mermaid
sequenceDiagram
participant Agent as "AI代理"
participant HTTP as "MCP HTTP API"
participant Tools as "MCP工具"
participant Scraper as "JwxtScraper"
participant Edu as "教务系统"
Agent->>HTTP : POST /api/mcp/tools/query_grades
HTTP->>Tools : 调用query_grades工具
Tools->>Scraper : 获取用户session
Scraper->>Edu : 查询成绩数据
Edu-->>Scraper : 返回成绩数据
Scraper-->>Tools : 格式化输出
Tools-->>HTTP : 返回结果
HTTP-->>Agent : JSON响应
```

**图示来源**
- [backend/app/api/mcp.py:96-146](file://backend/app/api/mcp.py#L96-L146)
- [backend/app/mcp/tools.py:44-82](file://backend/app/mcp/tools.py#L44-L82)
- [backend/mcp_server.py:22-35](file://backend/mcp_server.py#L22-L35)

**章节来源**
- [backend/app/api/mcp.py:1-195](file://backend/app/api/mcp.py#L1-L195)
- [backend/app/mcp/tools.py:1-310](file://backend/app/mcp/tools.py#L1-L310)
- [backend/mcp_server.py:1-35](file://backend/mcp_server.py#L1-L35)
- [openclaw/skill.json:1-107](file://openclaw/skill.json#L1-L107)

### 组件K：AI代理运行时服务（新增）
- **框架支持**：支持OpenAI Agents SDK和LangGraph两种AI框架，提供统一的代理执行接口。
- **自动降级**：当首选框架不可用时，自动降级到统一模型层。
- **事件循环处理**：针对LangGraph的事件循环冲突进行特殊处理。
- **工具集成**：通过统一模型提供层支持工具调用和RAG功能。

```mermaid
flowchart TD
Start(["AI代理请求"]) --> CheckFramework["检查框架可用性"]
CheckFramework --> |OpenAI Agents| RunOpenAI["运行OpenAI Agents"]
CheckFramework --> |LangGraph| RunLangGraph["运行LangGraph"]
CheckFramework --> |未知框架| Fallback["降级到统一模型层"]
RunOpenAI --> Success{"执行成功?"}
RunLangGraph --> Success
Success --> |是| ReturnResult["返回执行结果"]
Success --> |否| Fallback
Fallback --> ReturnFallback["返回降级结果"]
ReturnResult --> End(["结束"])
ReturnFallback --> End
```

**图示来源**
- [backend/app/services/agent_runtime.py:40-147](file://backend/app/services/agent_runtime.py#L40-L147)

**章节来源**
- [backend/app/services/agent_runtime.py:1-177](file://backend/app/services/agent_runtime.py#L1-L177)

### 组件L：统一模型提供层（新增）
- **抽象设计**：统一不同AI模型提供商的差异，提供一致的接口。
- **多提供商支持**：支持Qwen、OpenAI、LiteLLM等多种模型提供商。
- **自动回退**：主提供商失败时自动回退到QwenProvider。
- **用户偏好**：支持按用户设置不同的模型提供商和模型。

```mermaid
classDiagram
class BaseProvider {
<<abstract>>
+bool available
+chat(messages, temperature) Dict
+chat_stream(messages, temperature, education_context) Generator
+chat_with_tools(messages, tools_context) Dict
+chat_with_rag(question, context, conversation_history) Dict
+generate_embedding(text) List[float]
}
class QwenProvider {
+chat(messages, temperature) Dict
+chat_stream(messages, temperature, education_context) Generator
+chat_with_tools(messages, tools_context) Dict
+chat_with_rag(question, context, conversation_history) Dict
+generate_embedding(text) List[float]
}
class LiteLLMProvider {
+chat(messages, temperature) Dict
+chat_stream(messages, temperature, education_context) Generator
+chat_with_tools(messages, tools_context) Dict
+chat_with_rag(question, context, conversation_history) Dict
+generate_embedding(text) List[float]
}
class UnifiedModelProvider {
+BaseProvider primary
+BaseProvider fallback
+chat(messages, temperature) Dict
+chat_stream(messages, temperature, education_context) Generator
+chat_with_tools(messages, tools_context) Dict
+chat_with_rag(question, context, conversation_history) Dict
+generate_embedding(text) List[float]
}
BaseProvider <|-- QwenProvider
BaseProvider <|-- LiteLLMProvider
BaseProvider <|-- UnifiedModelProvider
```

**图示来源**
- [backend/app/services/model_provider.py:20-272](file://backend/app/services/model_provider.py#L20-L272)

**章节来源**
- [backend/app/services/model_provider.py:1-299](file://backend/app/services/model_provider.py#L1-L299)

### 组件M：LangChain、LangGraph、OpenAI集成（新增）
- **LangChain集成**：通过langchain-openai适配器支持OpenAI模型调用。
- **LangGraph编排**：使用StateGraph实现简单的对话编排。
- **OpenAI API**：支持通过OPENAI_API_KEY配置OpenAI模型。
- **事件循环处理**：针对LangGraph的事件循环冲突进行特殊处理。

**章节来源**
- [backend/app/services/agent_runtime.py:102-147](file://backend/app/services/agent_runtime.py#L102-L147)
- [backend/requirements.txt:26-36](file://backend/requirements.txt#L26-L36)

## 依赖分析
后端依赖包括Web框架、HTTP请求、HTML解析、缓存、向量数据库、**AI框架集成**（LangChain、LangGraph、OpenAI）、**统一模型提供层**（DashScope、LiteLLM）、数据处理、MCP协议支持等模块；前端依赖包括UI组件、图表库、SDK等。

```mermaid
graph LR
BE["后端(main.py)"] --> Req["requirements.txt"]
Req --> FastAPI["FastAPI"]
Req --> Requests["requests/aiohttp"]
Req --> BS4["BeautifulSoup4/lxml"]
Req --> RedisDep["redis"]
Req --> MilvusDep["pymilvus"]
Req --> LangChain["langchain==0.3.14"]
Req --> LangGraph["langgraph==0.4.5"]
Req --> LangChainOpenAI["langchain-openai==0.3.16"]
Req --> OpenAI["openai==1.59.6"]
Req --> DashScope["dashscope==1.20.11"]
Req --> LiteLLM["litellm==1.61.15"]
Req --> Tools["pandas/numpy"]
Req --> MCP["mcp==1.6.0"]
Req --> OpenAIAgents["openai-agents==0.0.15"]
FE["前端(next)"] --> Pkg["package.json"]
Pkg --> UI["Shadcn/UI + Tailwind"]
Pkg --> Charts["Recharts"]
Pkg --> SDKs["AWS S3/Supabase/Coze"]
```

**图示来源**
- [backend/requirements.txt:1-58](file://backend/requirements.txt#L1-L58)
- [package.json:12-68](file://package.json#L12-L68)

**章节来源**
- [backend/requirements.txt:1-58](file://backend/requirements.txt#L1-L58)
- [package.json:12-68](file://package.json#L12-L68)

## 性能考虑
- 爬虫并发与限流：对目标站点施加合理延时与并发上限，避免触发反爬机制。
- 缓存策略：热点数据（验证码、登录态、查询结果）使用Redis缓存，设置合理TTL与失效策略。
- 向量检索优化：Milvus索引参数（如nlist）与metric_type需结合数据规模与查询延迟权衡。
- 异步化：将耗时操作异步化，减少请求阻塞。
- CDN与对象存储：静态资源与大文件使用MinIO/对象存储，结合CDN加速。
- **新增** AI代理性能：AI代理运行时采用异步模式，避免阻塞主进程；合理设置AI模型参数与温度值。
- **新增** 模型提供层优化：统一模型提供层支持缓存和回退机制，提高整体可用性。
- **新增** LangGraph优化：LangGraph编排使用简单状态机，避免复杂的并发问题。

## 故障排查指南
- 登录失败：检查VPN/校园网连通性、验证码是否过期、服务器选择是否一致。
- 验证码异常：确认验证码接口可达、会话是否正确传递、超时设置是否合理。
- Milvus连接失败：检查容器健康状态、网络连通、索引创建是否成功。
- Redis不可用：确认容器健康、连接参数、集群模式（如使用Sentinel/Cluster）。
- 前端静态资源404：确认Nginx配置与构建产物路径。
- **新增** MCP工具调用失败：检查工具名称是否正确、参数格式是否符合Schema、用户是否已登录。
- **新增** OpenClaw集成问题：确认skill.json配置正确、MCP服务器可访问、工具映射表完整。
- **新增** AI代理运行时问题：检查OPENAI_API_KEY配置、框架依赖是否安装、事件循环冲突。
- **新增** 模型提供层问题：检查MODEL_PROVIDER环境变量、API密钥配置、模型可用性。
- **新增** LangGraph运行失败：检查OPENAI_API_KEY、事件循环状态、内存使用情况。

**章节来源**
- [backend/test_login.py:1-152](file://backend/test_login.py#L1-L152)
- [backend/test_scraper.py:1-280](file://backend/test_scraper.py#L1-L280)
- [backend/docker-compose.yml:1-167](file://backend/docker-compose.yml#L1-L167)
- [frontend/Dockerfile:1-33](file://frontend/Dockerfile#L1-L33)
- [backend/app/api/mcp.py:96-146](file://backend/app/api/mcp.py#L96-L146)
- [openclaw/skill.json:1-107](file://openclaw/skill.json#L1-L107)
- [backend/app/services/agent_runtime.py:102-147](file://backend/app/services/agent_runtime.py#L102-L147)
- [backend/app/services/model_provider.py:196-207](file://backend/app/services/model_provider.py#L196-L207)

## 结论
本项目已具备接入第三方服务的良好基础：清晰的API边界、可扩展的爬虫模块、成熟的向量与缓存能力，并**新增了完整的MCP集成、AI代理运行时服务以及统一的模型提供层**，支持多种AI框架的标准化交互。建议在生产环境中重点补齐以下能力：Redis集群与高可用、etcd配置中心、消息队列与异步任务、分布式追踪与监控告警、OAuth与API密钥管理、以及完善的CI/CD与容器编排方案。

## 附录
- 快速启动与测试：参考Windows部署文档中的步骤与端点，验证健康检查、验证码与登录流程。
- 构建与启动脚本：前端构建与后端服务启动脚本可用于本地开发与部署。
- **新增** MCP集成测试：可通过HTTP API或OpenClaw客户端测试MCP工具调用。
- **新增** OpenClaw技能配置：完整的技能描述、工具定义和使用示例。
- **新增** AI代理运行时测试：通过HTTP API测试OpenAI Agents SDK和LangGraph框架。
- **新增** 模型管理测试：通过模型管理API切换不同的AI模型提供商。

**章节来源**
- [README-Windows.md:171-211](file://README-Windows.md#L171-L211)
- [scripts/build.sh:1-18](file://scripts/build.sh#L1-L18)
- [scripts/start.sh:1-18](file://scripts/start.sh#L1-L18)
- [README.md:151-185](file://README.md#L151-L185)
- [openclaw/skill.json:1-107](file://openclaw/skill.json#L1-L107)
- [backend/app/api/agents.py:1-40](file://backend/app/api/agents.py#L1-L40)
- [backend/app/api/models.py:1-78](file://backend/app/api/models.py#L1-L78)