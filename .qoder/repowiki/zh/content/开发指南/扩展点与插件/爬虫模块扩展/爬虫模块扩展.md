...
## 更新摘要
**变更内容**
- **表格展开功能模块**：新增 `_expand_table_rows()` 方法，专门处理复杂HTML表格结构中的rowspan合并，提升爬虫对教育系统数据的解析能力
- **会话验证机制**：新增 `_check_session_valid` 方法，用于检测响应内容是否表示会话已失效（被踢回登录页）
- **URL修正机制**：改进 `base_url` 处理逻辑，确保URL拼接时保持正确的斜杠格式
- **相对链接处理**：在教师查询中修复相对链接的处理，避免与base_url重复拼接
- **调试基础设施优化**：新增详细的HTML调试输出功能，包括成绩查询、培养方案查询、个人信息查询的调试能力
- **统一POST请求策略**：所有需要表单提交的查询接口统一使用POST请求，提升数据抓取的稳定性和准确性
- **编码处理增强**：改进 `_fix_encoding` 方法的编码检测机制，通过严格的UTF-8检测提升编码处理的可靠性和准确性
- **智能爬取增强**：DeepCrawler类新增了更完善的页面分析、表单提交和HTML保存功能
...
16. [会话验证机制](#会话验证机制)
17. [URL修正与相对链接处理](#url修正与相对链接处理)
18. [智能爬取增强功能](#智能爬取增强功能)
19. [表格展开功能模块](#表格展开功能模块)
20. [依赖分析](#依赖分析)
21. [性能考虑](#性能考虑)
22. [故障排查指南](#故障排查指南)
23. [结论](#结论)
...
- **表格展开功能模块**：新增的 `_expand_table_rows()` 方法专门处理复杂HTML表格结构中的rowspan合并，显著提升爬虫对教育系统数据的解析能力
- **会话验证机制**：新增的会话状态检测功能，确保数据抓取过程中的会话有效性
- **深度优先智能爬虫系统**：新增的DeepCrawler类提供5层深度探索、自动页面分析、表单提交和HTML保存功能
- **HTML调试输出功能**：新增成绩、培养方案、个人信息的HTML调试输出能力，便于问题诊断和数据分析
- **统一POST请求策略**：所有需要表单提交的查询接口统一使用POST请求，提升数据抓取的稳定性和准确性
- **URL修正与相对链接处理**：改进的URL处理机制，确保正确的URL拼接和相对链接解析
- **编码处理机制**（GBK/UTF-8/GB18030 混合场景自动修复）
- **智能爬取增强功能**：DeepCrawler类的页面分析、表单识别和数据提取能力
- **调试基础设施优化**：详细的日志记录和HTML保存机制，提升开发和维护效率
...
- [scraper.py:1-1704](file://backend/scraper.py#L1-L1704)
...
**章节来源**
- [scraper.py:13-1704](file://backend/scraper.py#L13-L1704)
...
+_expand_table_rows(table) List[]str~~
+get_captcha() bytes
...
- **表格展开功能适配**：新增的 _expand_table_rows() 方法可以处理复杂的rowspan合并，适用于各种教育系统表格结构。
...
- **表格展开处理**：新增的 _expand_table_rows() 方法专门处理rowspan合并，确保复杂表格的正确解析。
...
- [scraper.py:1092-1704](file://backend/scraper.py#L1092-L1704)
...
**章节来源**
- [scraper.py:23-46](file://backend/scraper.py#L23-L46)
...
**新增** JwxtScraper 类新增了 `_check_session_valid` 方法，用于检测响应内容是否表示会话已失效（被踢回登录页）。
...
#### 工作流程
1. **内容检查**：检查HTML响应内容是否包含特定的登录页面特征
2. **特征匹配**：检测是否包含 'LoginToXk'、'USERNAME'、'PASSWORD' 等关键字
3. **会话状态判断**：如果匹配到登录页面特征，判断为会话已过期
4. **日志记录**：记录会话状态检查结果，便于调试和问题诊断
5. **返回值处理**：返回布尔值表示会话有效性

#### 会话验证优势
- **自动检测**：无需手动检查会话状态，自动识别登录页面特征
- **准确性高**：通过特定的关键字匹配，避免误判
- **调试友好**：详细的日志记录帮助开发者快速定位会话问题
- **安全性提升**：及时发现会话过期问题，避免无效请求
...
**章节来源**
- [scraper.py:48-54](file://backend/scraper.py#L48-L54)
- [scraper.py:95-183](file://backend/scraper.py#L95-L183)
...
#### 工作流程
1. **内容检查**：检查HTML响应内容是否包含特定的登录页面特征
2. **特征匹配**：检测是否匹配到 'LoginToXk'、'USERNAME'、'PASSWORD' 等关键字
3. **会话状态判断**：如果匹配到登录页面特征，判断为会话已过期
4. **日志记录**：记录会话状态检查结果，便于调试和问题诊断
5. **返回值处理**：返回布尔值表示会话有效性
...
**新增** 在教师查询中修复了相对链接的处理，避免与base_url重复拼接：
...
- [scraper.py:1203-1209](file://backend/scraper.py#L1203-L1209)
...
**重大改进** DeepCrawler 类经过重要增强，新增了更完善的页面分析、表单识别和数据提取能力。

### 增强的页面分析功能

#### 更精确的链接提取
- **URL规范化**：新增了对相对URL的规范化处理，避免与base_url重复拼接
- **域名白名单**：确保只提取同域名的链接，避免跨域访问
- **链接去重**：通过集合去重，避免重复链接的处理

#### 改进的表单识别
- **表单属性增强**：新增了对表单action、method、inputs的完整提取
- **输入字段分类**：区分input、select、textarea等不同类型的输入字段
- **选项数据提取**：对select类型的选项进行完整提取，包括value和text

#### iframe处理增强
- **src地址提取**：新增了对iframe src属性的提取和处理
- **相对URL转换**：将相对URL转换为绝对URL
- **域名验证**：确保iframe地址也在同域名范围内

### 增强的表单提交功能

#### 更智能的表单数据准备
- **字段映射优化**：改进了字段名称到值的映射机制
- **数据类型处理**：根据字段类型进行相应的数据格式化
- **空值处理**：对没有值的字段进行空字符串处理

#### 提交策略优化
- **请求类型判断**：根据method选择GET或POST请求
- **超时控制**：设置10秒超时，避免长时间等待
- **错误处理**：对表单提交失败的情况进行详细记录和处理

### 增强的数据提取功能

#### 更精确的页面类型检测
- **关键词匹配**：新增了对页面关键词的精确匹配机制
- **多关键词支持**：支持多个关键词的组合匹配
- **匹配优先级**：根据关键词的重要性确定匹配优先级
- **类型返回**：返回匹配到的页面类型

#### 改进的数据提取策略
- **表格识别增强**：新增了对表格的精确识别和提取
- **HTML片段保存**：将提取的数据表格保存为HTML片段
- **数据验证**：对提取的数据进行格式验证和清洗

**章节来源**
- [deep_crawler.py:84-110](file://deep_crawler.py#L84-L110)
- [deep_crawler.py:112-160](file://deep_crawler.py#L112-L160)
- [deep_crawler.py:162-175](file://deep_crawler.py#L162-L175)
- [deep_crawler.py:355-367](file://deep_crawler.py#L355-L367)

## 表格展开功能模块

**新增** JwxtScraper 类新增了 `_expand_table_rows()` 方法，专门处理复杂HTML表格结构中的rowspan合并问题，显著提升了爬虫对教育系统数据的解析能力。

### _expand_table_rows 方法详解
...
#### 工作流程
1. **表格遍历**：遍历表格中的每一行(tr)
2. **单元格处理**：处理每个单元格(td/th)，提取文本内容
3. **rowspan处理**：检测rowspan属性，将单元格内容复制到后续行
4. **colspan处理**：检测colspan属性，跳过相应数量的列
5. **行列对齐**：确保所有行都有相同的列数
6. **结果返回**：返回二维数组格式的表格数据

#### 表格展开优势
- **rowspan合并**：自动处理rowspan导致的单元格内容重复问题
- **复杂表格支持**：能够处理教育系统中复杂的表格结构
- **数据完整性**：确保展开后的表格数据完整且结构清晰
- **兼容性**：适用于各种HTML表格格式，包括带边框和无边框表格
- **性能优化**：通过一次遍历完成表格展开，避免多次DOM操作

#### 应用场景
该方法在以下教育系统数据抓取场景中发挥重要作用：
- **培养方案表格**：处理课程列表中的rowspan合并
- **课表表格**：处理时间安排中的rowspan合并
- **成绩表格**：处理学分统计中的rowspan合并
- **学籍卡片**：处理个人信息表格中的rowspan合并

#### 使用示例
```python
# 在培养方案解析中使用
target_table = soup.find('table', {'id': 'course-table'})
if target_table:
...
- [scraper.py:972-976](file://backend/scraper.py#L972-L976)
- [scraper.py:1076-1080](file://backend/scraper.py#L1076-L1080)

### 表格展开在各数据抓取方法中的应用

#### 培养方案数据展开
```python
# 在 get_my_training_plan() 中使用
target_table = soup.find('table', {'id': 'pyfa-table'})
if target_table:
    expanded_rows = self._expand_table_rows(target_table)
    # 处理展开后的表格数据
    for row in expanded_rows:
...
#### 学业进度数据展开
```python
# 在 get_academic_progress() 中使用
progress_table = soup.find('table', {'id': 'progress-table'})
if progress_table:
...
if schedule_table:
    expanded_rows = self._expand_table_rows(schedule_table)
    # 解析课程时间、教室、教师等信息
    pass
```
...
```python
# 在 get_grades() 中使用
grades_table = soup.find('table', {'id': 'grades-table'})
if grades_table:
    expanded_rows = self._expand_table_rows(grades_table)
    # 提取课程成绩、绩点等信息
    pass
```
...
- **会话验证优化**：新增的会话验证机制通过关键字匹配快速检测会话状态，避免无效请求。
- **URL处理优化**：通过统一的 base_url 处理机制，避免了 URL 构造错误，减少了重试和错误处理的开销。
...
- **相对链接处理**：在教师查询中修复的相对链接处理机制，避免了重复的URL拼接操作，提升了性能。
- **表格展开性能**：新增的 _expand_table_rows() 方法通过高效的rowspan处理算法，提升了复杂表格的解析性能，减少了多次DOM操作的开销。
...
- **会话验证问题**：如果遇到会话过期问题，检查 _check_session_valid 方法的返回值，确认是否正确检测到登录页面特征。
...
- **相对链接问题**：如果教师查询中的链接解析失败，检查相对链接处理逻辑，确保正确的URL拼接。
...
- **表格展开问题**：如果培养方案或课表解析失败，检查 _expand_table_rows() 方法的执行情况，确认rowspan处理是否正确。
- **表格展开性能问题**：如果表格展开过程耗时过长，检查表格结构复杂度，考虑优化表格解析逻辑。
...
## 结论
JwxtScraper 提供了完整的教务系统数据抓取能力，配合 FastAPI 接口层与教育选项工具，能够快速构建面向 AI 的 RAG 系统。**新增的深度优先智能爬虫系统（DeepCrawler）提供了革命性的自动化爬取能力，通过5层深度探索、自动页面分析、表单提交和HTML保存功能，显著提升了爬取效率和数据质量。**通过扩展基础 URL、适配页面结构、完善异常处理与缓存策略，可有效应对多校区、多系统的差异化需求。**新增的增强版 _fix_encoding 方法通过严格的 UTF-8 检测机制，显著提升了编码处理的可靠性，解决了 GBK/UTF-8/GB18030 混合编码问题，进一步增强了数据抓取的稳定性和准确性。**新增的会话验证机制通过关键字匹配快速检测会话状态，避免无效请求。**新增的HTML调试输出功能为关键数据抓取方法提供了详细的HTML文件保存能力，便于问题诊断和数据分析。**统一的POST请求使用策略提升了数据抓取的稳定性和准确性，所有需要表单提交的查询接口都统一使用POST请求。**最重要的功能增强是新增的 _expand_table_rows() 方法，专门处理复杂HTML表格结构中的rowspan合并，显著提升了培养方案、课表等复杂表格的解析能力和性能。**新增的相对链接处理机制修复了教师查询中的URL拼接问题，避免了重复的URL拼接操作。**建议在生产环境中引入 Redis 缓存、代理池与更完善的监控体系，持续优化性能与稳定性。
...
- **实时测试**：test_scraper_live.py 提供真实的登录和爬取测试，使用真实HTML验证解析逻辑。
