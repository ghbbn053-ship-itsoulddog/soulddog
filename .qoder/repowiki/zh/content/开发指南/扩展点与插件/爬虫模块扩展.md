...
- [scraper.py:1-1228](file://backend/scraper.py#L1-L1228)
...
**章节来源**
- [scraper.py:13-1228](file://backend/scraper.py#L13-L1228)
...
- [scraper.py:1092-1228](file://backend/scraper.py#L1092-L1228)
...
### _fix_encoding 方法详解
JwxtScraper 类新增了 `_fix_encoding` 方法，专门用于处理教务系统中常见的 GBK/UTF-8 混合编码问题，确保数据解析的准确性。
...
#### 工作流程
1. **UTF-8 优先检测**：首先尝试使用 UTF-8 解码响应内容
2. **异常回退机制**：若 UTF-8 解码失败，抛出 UnicodeDecodeError 异常
3. **GBK 兼容处理**：使用 gb18030 编码（兼容 GBK 和 GB2312）重新解码
4. **统一编码设置**：将响应对象的 encoding 属性设置为最终确定的编码格式
...
#### 编码处理优势
- **自动适应**：无需手动指定编码，自动检测并选择最适合的编码格式
- **兼容性强**：gb18030 编码兼容 GBK 和 GB2312，提高解析成功率
- **透明处理**：对上层调用者完全透明，无需关心底层编码细节
- **错误预防**：避免因编码不匹配导致的乱码和解析失败

**章节来源**
- [scraper.py:22-29](file://backend/scraper.py#L22-L29)
- [scraper.py:50](file://backend/scraper.py#L50)
- [scraper.py:76](file://backend/scraper.py#L76)
- [scraper.py:128](file://backend/scraper.py#L128)
- [scraper.py:184](file://backend/scraper.py#L184)
- [scraper.py:333](file://backend/scraper.py#L333)
- [scraper.py:502](file://backend/scraper.py#L502)
- [scraper.py:575](file://backend/scraper.py#L575)
- [scraper.py:710](file://backend/scraper.py#L710)
- [scraper.py:806](file://backend/scraper.py#L806)
- [scraper.py:875](file://backend/scraper.py#L875)
- [scraper.py:944](file://backend/scraper.py#L944)
- [scraper.py:998](file://backend/scraper.py#L998)
- [scraper.py:1061](file://backend/scraper.py#L1061)
- [scraper.py:1107](file://backend/scraper.py#L1107)
...
- **编码优化**：_fix_encoding 方法自动处理编码问题，避免因编码错误导致的解析失败和重试开销。
...
- **编码问题**：如出现乱码或解析失败，检查 _fix_encoding 方法是否正确应用，确认响应编码是否被正确设置。
...
## 结论
JwxtScraper 提供了完整的教务系统数据抓取能力，配合 FastAPI 接口层与教育选项工具，能够快速构建面向 AI 的 RAG 系统。通过扩展基础 URL、适配页面结构、完善异常处理与缓存策略，可有效应对多校区、多系统的差异化需求。**新增的 _fix_encoding 方法显著提升了编码处理的可靠性，解决了 GBK/UTF-8 混合编码问题，进一步增强了数据抓取的稳定性和准确性。**建议在生产环境中引入 Redis 缓存、代理池与更完善的监控体系，持续优化性能与稳定性。
