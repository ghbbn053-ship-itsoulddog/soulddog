...
- **新增** 增强的调试功能，包括URL跟踪、HTTP状态码记录、响应URL监控、HTML内容长度统计和内容预览功能
...
- [backend/scraper.py:1-1303](file://backend/scraper.py#L1-L1303)
...
- [backend/scraper.py:13-1303](file://backend/scraper.py#L13-L1303)
- [backend/app/api/chat.py:1-249](file://backend/app/api/chat.py#L1-L249)
- [backend/education_options.py:1-420](file://backend/education_options.py#L1-L420)
- [backend/app/services/vector_store.py:1-164](file://backend/app/services/vector_store.py#L1-L164)
- [backend/app/models/base.py:1-29](file://backend/app/models/base.py#L1-L29)
...
- 调试技巧
  ...
  - **新增** 检查 `/tmp/debug_personal_info.html` 文件内容，分析解析问题
...
- **问题HTML响应自动保存**：将问题页面的HTML内容自动保存到 `/tmp/debug_personal_info.html` 文件
...
#### 3. 检查自动保存的HTML文件
当出现问题时，系统会自动将HTML内容保存到 `/tmp/debug_personal_info.html` 文件中，便于手动分析：
...
Analyze --> |异常| CheckFile["检查 /tmp/debug_personal_info.html"]
...
#### 2. 监控HTML文件变化
在调试过程中，观察 `/tmp/debug_personal_info.html` 文件的变化：
- 文件是否存在
- 文件内容是否更新
- HTML结构是否符合预期
- 特殊字符是否正确显示
...
#### 4. 分析问题页面
当调试功能检测到异常时：
- 打开 `/tmp/debug_personal_info.html` 文件
- 检查页面结构和内容
- 对比预期的HTML结构
- 识别解析失败的原因
...
**章节来源**
- [backend/scraper.py:238-1303](file://backend/scraper.py#L238-L1303)
...
- 排查步骤
  ...
  - **新增** 检查 `/tmp/debug_personal_info.html` 文件，分析解析问题
...
- [backend/scraper.py:138-1303](file://backend/scraper.py#L138-L1303)
...
- **新增** 诊断工具
  ...
  - 响应内容长度与编码关系验证
  - HTML结构变化监控
  - **新增** `/tmp/debug_personal_info.html` 文件分析
...
- **新增** 排查步骤
  ...
  - 验证 `/tmp/debug_personal_info.html` 文件是否正确保存
  - 分析HTML内容预览，确认包含预期信息
...
- **新增** 利用增强的调试功能进行调试
  ...
  - **新增** 重点关注调试功能相关的日志信息
  - **新增** 检查 `/tmp/debug_personal_info.html` 文件内容
  - **新增** 验证统一POST请求策略的调试效果

**章节来源**
- [backend/main.py:135-385](file://backend/main.py#L135-L385)
- [backend/scraper.py:22-1303](file://backend/scraper.py#L22-L1303)
...
## 结论
通过在FastAPI路由、JwxtScraper、AI对话与向量库等关键路径设置断点，结合INFO/WARNING/ERROR级别的日志输出与HTTPException的统一错误处理，能够高效定位与解决验证码、登录、数据爬取与RAG相关问题。**新增的增强调试功能**（包括URL跟踪、HTTP状态码记录、响应URL监控、HTML内容长度统计、内容预览和问题HTML响应自动保存）大大增强了系统的可观测性和问题诊断能力，特别是针对个人信息爬取功能的问题诊断。**编码检测机制的重大改进**为整个系统提供了更稳健的文本处理能力，有效解决了GBK/UTF-8混合场景下的编码问题。**统一的POST请求策略和超时配置**进一步提升了系统的稳定性和性能表现。配合性能监控与容器化部署，可进一步提升系统的稳定性与可观测性。
