...
- [deep_crawl/crawl_tree.json](file://deep_crawl/crawl_tree.json)
...
## 更新摘要
**所做更改**
- 新增智能爬虫测试体系章节，详细介绍自动化测试流程
- 扩展HTML保存分析功能，包含完整的HTML文件结构和分析能力
- 增强深度爬取与HTML分析章节，展示树形爬取和数据提取能力
- 新增部署检查与运维章节，提供自动化部署验证功能
- 更新实时爬虫测试功能，支持与真实教务系统交互的测试能力
...
- [backend/scraper.py:1-1258](file://backend/scraper.py#L1-L1258)
...
+get_schedule(semester) Dict
+get_training_plan(department, grade, major) Dict
...
class LiveCrawlerTester {
+session Session
+base_url String
+scraper JwxtScraper
+test_login_and_scrape() void
+test_profile() void
+test_grades() void
+test_schedule() void
+test_training_plan() void
+test_academic_progress() void
+save_real_html() void
}
...
LiveCrawlerTester --> JwxtScraper : "使用"
```
...
- [backend/app/services/qwen_service.py:15-511](file://backend/app/services/qwen_service.py#L15-L511)
- [smart_crawler_test.py:25-416](file://smart_crawler_test.py#L25-L416)
- [deep_crawler.py:27-589](file://deep_crawler.py#L27-L589)
- [test_scraper_live.py:19-241](file://test_scraper_live.py#L19-L241)
...
**图表来源**
- [backend/scraper.py:23-55](file://backend/scraper.py#L23-L55)
...
| query_schedule | 查询课表 | semester | 课程安排、时间地点 |
...
新增的实时爬虫测试器提供了与真实教务系统交互的能力：

```mermaid
flowchart TD
LiveStart[开始实时测试] --> GetUserInfo[获取用户输入]
GetUserInfo --> CreateSession[创建会话]
CreateSession --> DownloadCaptcha[下载验证码]
DownloadCaptcha --> SaveCaptcha[保存验证码图片]
SaveCaptcha --> InputCaptcha[输入验证码]
InputCaptcha --> RealLogin[真实系统登录]
RealLogin --> TestProfile[测试个人信息]
TestProfile --> TestGrades[测试成绩查询]
TestGrades --> TestSchedule[测试课表查询]
TestSchedule --> TestTrainingPlan[测试培养方案]
TestTrainingPlan --> TestProgress[测试学业进度]
TestProgress --> SaveHTML[保存原始HTML]
SaveHTML --> LiveComplete[实时测试完成]
```
...
- [deep_crawl/crawl_tree.json:1-200](file://deep_crawl/crawl_tree.json#L1-L200)
...
新增的部署检查脚本提供了完整的自动化部署验证功能：

```mermaid
flowchart TD
DeployStart[开始部署检查] --> CheckContainers[检查Docker容器状态]
CheckContainers --> PullCode[拉取最新代码]
PullCode --> RestartBackend[重启后端服务]
RestartBackend --> CheckLogs[查看后端日志]
CheckLogs --> AnalyzeLogs[分析关键日志]
AnalyzeLogs --> Success[部署检查完成]
Success --> Tips[显示使用提示]
Tips --> DeployComplete[部署完成]
```
...
| 检查项目 | 功能描述 | 实现方式 |
|---------|---------|---------|
| 容器状态检查 | 验证Docker容器运行状态 | `docker-compose ps` |
| 代码更新检查 | 拉取最新代码并重启服务 | `git pull origin main` |
| 日志分析 | 关键日志过滤和分析 | `docker-compose logs` |
| 编码修复验证 | 检查编码修复效果 | `grep "【编码】"` |
| 登录成功验证 | 确认登录功能正常 | `grep "登录成功"` |
| 数据解析验证 | 验证各模块数据解析 | `grep "【培养方案】"`、`grep "成功获取.*条成绩记录"` |
...
Dev->>Script : 执行部署检查
Script->>Docker : 检查容器状态
Script->>Script : 拉取最新代码
Script->>Docker : 重启后端服务
Script->>Backend : 查看服务日志
Backend->>Logs : 输出关键日志
Script->>Logs : 分析日志内容
Logs-->>Script : 返回检查结果
Script-->>Dev : 显示部署状态
...
**图表来源**
- [check-deployment.sh:9-67](file://check-deployment.sh#L9-L67)
...
- [check-deployment.sh:14-27](file://check-deployment.sh#L14-L27)
...
7. **部署自动化**：新增部署检查脚本实现一键部署验证
8. **实时测试能力**：支持与真实教务系统交互的实时测试功能
...
7. **部署检查自动化**：将部署检查脚本集成到CI/CD流程中

该项目展现了现代教育技术应用的先进理念，通过技术创新提升了教务服务的智能化水平，为类似项目的开发提供了宝贵的参考经验。新增的智能爬虫测试功能、深度分析能力和部署检查脚本进一步增强了系统的可靠性和可维护性，为项目的长期稳定运行奠定了坚实基础。