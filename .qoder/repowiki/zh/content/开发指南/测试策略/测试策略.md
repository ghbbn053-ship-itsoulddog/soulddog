...
## 简介
本测试策略面向"智能教务系统AI助手"项目，目标是建立覆盖单元测试、集成测试与端到端测试的完整测试体系，重点保障登录功能、爬虫模块、学术进度解析与课表调试的稳定性与可靠性。策略涵盖：
...
- **新增**：学术进度测试模块和课表调试测试模块，强化数据解析验证能力
- **新增**：安全隔离测试模块，确保用户会话安全与数据隔离
- **新增**：会话存储单例测试模块，验证会话管理的正确性与一致性

## 项目结构
后端采用FastAPI，提供教务系统数据爬取与AI对话能力；前端基于Next.js；测试脚本位于backend/tests目录；容器编排通过docker-compose定义数据库、缓存与向量库等依赖。**新增**学术进度测试模块、课表调试测试模块、安全隔离测试模块和会话存储单例测试模块，提供更完善的测试覆盖。
...
SCRAPER["爬虫测试"]
ACADEMIC["学术进度测试"]
SCHEDULE_DEBUG["课表调试测试"]
SCHEDULE_PARSE["课表解析测试"]
SECURITY["安全隔离测试"]
SESSION_STORE["会话存储单例测试"]
END
LOGIN --> BE
SCRAPER --> BE
ACADEMIC --> BE
...
- [backend/test_academic_progress.py:1-109](file://backend/test_academic_progress.py#L1-L109)
- [backend/test_schedule_debug.py:1-127](file://backend/test_schedule_debug.py#L1-L127)
- [backend/test_schedule_parsing.py:1-169](file://backend/test_schedule_parsing.py#L1-L169)
...
- 爬虫模块：封装对教务系统的数据抓取逻辑，包括个人信息、成绩、课表、培养方案、学术进度等
...
- **新增**：学术进度测试模块，专门验证学业进度数据的解析准确性
- **新增**：课表调试测试模块，提供课表解析逻辑的详细调试能力
- **新增**：课表解析测试模块，验证课表数据的完整解析流程
- **新增**：安全隔离模块，确保用户会话安全与数据访问控制
- **新增**：会话存储模块，提供进程级单例模式的会话管理服务
...
- [backend/test_academic_progress.py:12-109](file://backend/test_academic_progress.py#L12-L109)
- [backend/test_schedule_debug.py:1-127](file://backend/test_schedule_debug.py#L1-L127)
- [backend/test_schedule_parsing.py:12-169](file://backend/test_schedule_parsing.py#L12-L169)
...
- **新增**：学术进度解析：专门处理学业进度数据的提取与验证
- **新增**：课表调试：提供详细的课表解析过程跟踪与分析
- **新增**：安全隔离：强制用户名隔离，防止会话劫持与数据泄露
- **新增**：会话管理：提供进程级单例的会话存储服务
...
participant AcademicTest as "学术进度测试"
participant ScheduleDebug as "课表调试测试"
...
API->>Security : "强制用户名隔离检查"
Security->>SessionStore : "验证auth_session_id"
SessionStore-->>Security : "会话有效性"
Security-->>API : "隔离检查通过"
API->>Edu : "登录请求"
Edu-->>API : "登录结果"
API-->>Client : "登录成功/失败"
...
Edu-->>Scraper : "原始数据"
Scraper-->>API : "结构化数据"
API->>AcademicTest : "验证学业进度解析"
AcademicTest-->>API : "解析结果"
API->>ScheduleDebug : "调试课表解析"
ScheduleDebug-->>API : "调试信息"
API->>Store : "写入向量库"
Store-->>API : "写入完成"
API-->>Client : "聚合数据"
```
...
- [backend/app/services/session_store.py:192-201](file://backend/app/services/session_store.py#L192-L201)
...
### **新增**：学术进度测试模块
目标：验证学业进度数据的解析准确性与完整性
- 覆盖点
  - 总学分要求提取：从表格标题中解析总学分需求
  - 表格定位：通过特征字符串'需修读总学分'精确定位表格
  - 行解析：遍历表格行，识别合计行与数据行
  - 字段提取：课程性质、代码、名称、学分、建议修读学期等
  - 统计计算：累计已获学分，验证数据一致性
- 测试方法
  - 使用真实HTML文件进行端到端解析测试
  - 验证正则表达式匹配的准确性
...
- 参考实现
  - [backend/test_academic_progress.py:12-109](file://backend/test_academic_progress.py#L12-L109)
  - [backend/scraper.py:957-1047](file://backend/scraper.py#L957-L1047)
...
目标：提供详细的课表解析过程调试能力
- 覆盖点
  - 表格定位：通过id='kbtable'精确定位课表表格
  - 节次映射：验证节次文本到数字范围的转换
  - 行解析：跳过表头行，解析每节课的信息
  - 多课程处理：使用分隔符识别同一单元格中的多门课程
  - 字段提取：课程名称、教师、周次、地点、节次信息
- 测试方法
  - 读取真实课表HTML文件进行解析测试
  - 验证节次映射表的完整性
  - 检查多课程分隔符的正则表达式匹配
  - 断言每个课程块的字段提取准确性
- 参考实现
  - [backend/test_schedule_debug.py:1-127](file://backend/test_schedule_debug.py#L1-127)

**章节来源**
- [backend/test_schedule_debug.py:1-127](file://backend/test_schedule_debug.py#L1-127)

### **新增**：课表解析测试模块
目标：验证完整的课表解析流程与数据完整性
- 覆盖点
  - HTML文件读取：从指定路径读取真实课表数据
  - BeautifulSoup解析：验证HTML解析的正确性
  - 表格定位：查找id='kbtable'的课表表格
  - 节次映射验证：确保节次转换的准确性
  - 课程块分割：使用正则表达式分割多课程块
  - 字段精确提取：通过font标签的title属性提取字段
- 测试方法
  - 使用调试信息输出解析过程
  - 验证每个步骤的中间结果
  - 检查课程数据的完整性
...
### **新增**：安全隔离测试模块
目标：验证用户名隔离机制的正确性与安全性
- 覆盖点
  - 会话缺失：当缺少会话cookie时允许访问
  - 会话匹配：当auth_session_id与用户名匹配时允许访问
  - 会话不匹配：当auth_session_id与用户名不匹配时拒绝访问（403）
  - 无效会话：当auth_session_id不存在于会话存储时拒绝访问（401）
  - 兼容模式：旧的session_username cookie校验
- 测试方法
  - 构造不同类型的Request对象，模拟各种会话场景
  - 使用Mock的SessionStore模拟会话存储行为
  - 验证HTTPException的状态码与错误消息
  - 测试安全隔离函数的边界条件
- 参考实现
  - [backend/tests/test_security_isolation.py:1-57](file://backend/tests/test_security_isolation.py#L1-L57)
  - [backend/app/security.py:4-26](file://backend/app/security.py#L4-L26)
...
### **新增**：会话存储单例测试模块
目标：验证SessionStore单例模式的正确性与会话管理功能
- 覆盖点
  - 单例实例：多次调用get_session_store返回同一实例
  - 内存回退：当Redis不可用时使用内存存储
  - 用户会话存储：set_user_session/get_user_session的完整流程
  - 会话数据完整性：验证cookies和headers的序列化与反序列化
  - 会话标识符：确保JSESSIONID等关键标识符正确传递
- 测试方法
  - 验证单例模式的线程安全性和实例唯一性
  - 设置redis_available=False模拟内存存储场景
  - 使用requests.Session模拟真实的HTTP会话
  - 验证会话数据的完整性和正确性
  - 测试会话存储的增删改查操作
- 参考实现
  - [backend/tests/test_session_store_singleton.py:1-30](file://backend/tests/test_session_store_singleton.py#L1-L30)
...
- **新增**：测试模块依赖
  - BeautifulSoup：用于HTML解析和调试
  - 正则表达式：用于文本匹配和分割
  - 文件系统：用于读取真实HTML测试数据
  - Starlette：用于构建测试Request对象
  - Requests：用于模拟HTTP会话
...
subgraph "测试模块依赖"
AP["学术进度测试"] --> BS4["BeautifulSoup4"]
SD["课表调试测试"] --> BS4
SP["课表解析测试"] --> BS4
SEC["安全隔离测试"] --> ST["Starlette"]
SES["会话存储测试"] --> REQ["Requests"]
AP --> RE["正则表达式"]
SD --> RE
SP --> RE
SEC --> RE
...
- [docker-compose.yml:1-148](file://docker-compose.yml#L1-L148)
- [backend/test_academic_progress.py:6](file://backend/test_academic_progress.py#L6)
- [backend/test_schedule_debug.py:2](file://backend/test_schedule_debug.py#L2)
- [backend/test_schedule_parsing.py:7](file://backend/test_schedule_parsing.py#L7)
- [backend/tests/test_security_isolation.py:1](file://backend/tests/test_security_isolation.py#L1)
- [backend/tests/test_session_store_singleton.py:1](file://backend/tests/test_session_store_singleton.py#L1)
...
- **新增**：学术进度解析性能：优化正则表达式匹配效率，减少不必要的HTML解析
- **新增**：课表调试性能：合理使用BeautifulSoup解析，避免重复解析相同内容
- **新增**：安全隔离性能：优化会话存储访问频率，避免频繁的Redis查询
- **新增**：会话存储性能：合理设置会话TTL，平衡内存使用与性能
...
- **新增**：学术进度解析失败
  - 检查HTML文件路径是否正确
  - 验证正则表达式匹配是否准确
  - 确认表格特征字符串'需修读总学分'是否存在
- **新增**：课表解析异常
  - 确认id='kbtable'表格是否存在
  - 检查节次映射表的完整性
  - 验证多课程分隔符的正则表达式
- **新增**：课表调试问题
  - 检查kbcontent类div的识别是否正确
  - 确认font标签的title属性提取是否准确
  - 验证课程块分割的逻辑正确性
- **新增**：安全隔离问题
  - 检查auth_session_id cookie是否存在
  - 验证会话存储中是否存在对应的auth_session_id
  - 确认用户名与会话中的用户名是否匹配
- **新增**：会话存储问题
  - 检查Redis连接状态
  - 验证会话数据的序列化与反序列化
...
- [backend/test_academic_progress.py:16-109](file://backend/test_academic_progress.py#L16-L109)
- [backend/test_schedule_debug.py:6-127](file://backend/test_schedule_debug.py#L6-L127)
- [backend/test_schedule_parsing.py:18-169](file://backend/test_schedule_parsing.py#L18-L169)
- [backend/app/security.py:4-26](file://backend/app/security.py#L4-L26)
- [backend/app/services/session_store.py:192-201](file://backend/app/services/session_store.py#L192-L201)

## 结论
本测试策略围绕登录、爬虫与API三大核心模块，结合单元、集成与端到端测试，形成闭环的质量保障体系。**新增的学术进度测试模块、课表调试测试模块、课表解析测试模块、安全隔离测试模块和会话存储单例测试模块**进一步增强了系统的数据解析验证能力、安全性和会话管理可靠性。通过明确的测试用例设计、测试数据管理与自动化流程，能够有效提升系统的稳定性与可维护性。建议在CI/CD中集成测试任务，输出测试报告并纳入质量门禁。
...
- 并发与会话
  - 多用户同时登录与查询，验证会话隔离
- **新增**：学术进度测试用例
  - 总学分提取边界条件、表格定位策略验证、字段映射准确性
- **新增**：课表调试测试用例
  - 节次映射完整性、多课程分割准确性、字段提取精确性
- **新增**：安全隔离测试用例
  - 会话缺失场景、会话匹配场景、会话不匹配场景、无效会话场景
- **新增**：会话存储测试用例
  - 单例模式验证、内存回退验证、用户会话存储验证、会话数据完整性验证
- **新增**：部署验证测试
...
- **新增**：学术进度测试数据
  - 准备包含'需修读总学分'特征字符串的HTML文件
  - 创建不同格式的学业进度页面用于测试
- **新增**：课表测试数据
  - 准备标准课表HTML文件，包含kbtable表格
  - 创建多课程、单课程等不同场景的课表数据
- **新增**：安全隔离测试数据
  - 准备不同类型的Request对象，模拟各种会话场景
  - 创建Mock的SessionStore用于测试会话存储行为
- **新增**：会话存储测试数据
  - 准备requests.Session对象，模拟真实的HTTP会话
  - 创建不同配置的会话存储实例，测试内存与Redis模式
- **新增**：部署测试数据
  - 准备不同版本的部署脚本和配置文件
  - 创建测试用的HTML样例文件
...
- 流程建议
  ...
  - **新增**：学术进度测试：python backend/test_academic_progress.py
  - **新增**：课表调试测试：python backend/test_schedule_debug.py
  - **新增**：课表解析测试：python backend/test_schedule_parsing.py
  - **新增**：安全隔离测试：python backend/tests/test_security_isolation.py
  - **新增**：会话存储单例测试：python backend/tests/test_session_store_singleton.py
...
- [README-Windows.md:171-212](file://README-Windows.md#L171-L212)
- [backend/test_academic_progress.py:108](file://backend/test_academic_progress.py#L108)
- [backend/test_schedule_debug.py:126](file://backend/test_schedule_debug.py#L126)
- [backend/test_schedule_parsing.py:168](file://backend/test_schedule_parsing.py#L168)
- [backend/tests/test_security_isolation.py:1-57](file://backend/tests/test_security_isolation.py#L1-L57)
- [backend/tests/test_session_store_singleton.py:1-30](file://backend/tests/test_session_store_singleton.py#L1-L30)
- [backend/tests/conftest.py:1-11](file://backend/tests/conftest.py#L1-L11)
- [check-deployment.sh:1-69](file://check-deployment.sh#L1-L69)
- [test_scraper_live.py:1-241](file://test_scraper_live.py#L1-L241)
- [smart_crawler_test.py:1-416](file://smart_crawler_test.py#L1-L416)
