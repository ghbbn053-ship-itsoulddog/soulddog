...
- 前端 Next.js 项目的构建：依赖安装、TypeScript 编译、静态资源导出与 Nginx 托管。
...
- 前端 Next.js 构建链路
  ...
  - TypeScript 编译：遵循 frontend/tsconfig.json 与根 tsconfig.json 的编译选项。
  - 静态导出：next.config.ts 设置 output 为 export，构建产物位于 dist。
  - 样式与 PostCSS：使用 Tailwind PostCSS 插件。
  - Docker 化：前端 Dockerfile 在构建阶段执行 pnpm build，运行阶段使用 Nginx 提供静态服务。
- 后端 Node.js 服务器
...
- 静态资源处理
  - next.config.ts 将 output 设为 export，distDir 为 dist；允许特定远程图片域名。
  - PostCSS 使用 @tailwindcss/postcss 插件。
- Docker 化
  - 构建阶段：复制 package.json 与锁文件，安装依赖，复制源码并执行 pnpm build。
  - 运行阶段：复制 dist 至 Nginx HTML 目录，加载 nginx.conf，暴露 3000 端口。

```mermaid
flowchart TD
Start(["开始"]) --> Install["pnpm install<br/>使用冻结锁文件与离线模式"]
Install --> BuildNext["pnpm next build<br/>静态导出"]
BuildNext --> Dist["生成 dist/ 静态产物"]
Dist --> DockerBuild["Dockerfile 构建阶段<br/>pnpm build"]
DockerBuild --> NginxRun["运行阶段 Nginx 提供服务"]
NginxRun --> End(["完成"])
```
...
- 执行步骤
  - 进入项目根目录，执行 scripts/build.sh 完成依赖安装、Next.js 构建与 server.ts 打包。
  - 产物位置：
    - 前端静态产物：frontend/dist
    - Node.js 服务器：dist/server.js
- 产物验证
  - 前端：访问 http://localhost:3000（若使用 Docker Compose，前端容器映射 3000 端口）。
...
## 依赖关系分析
- 前端
  - next.config.ts 依赖 next 与图片远程模式配置。
  - tsconfig.json 与根 tsconfig.json 共同约束编译选项。
  - Dockerfile 依赖 pnpm 与 Nginx。
- 后端
...
FE_DOCKER --> FE_DIST["frontend/dist"]
...
## 性能考虑
- 代码分割与 Tree Shaking
  - Next.js 默认启用按路由的动态导入与 Tree Shaking；确保使用动态 import 与按需加载组件。
- 压缩与体积控制
  - 当前 build.sh 使用 --no-minify 与 --no-splitting，便于调试；生产建议开启压缩与合理拆分。
- 缓存与增量编译
  - TypeScript 配置启用 incremental；pnpm 使用锁定文件提升安装稳定性。
- 静态导出与 CDN
  - Next.js export 产物可配合 CDN 加速静态资源访问。
...
- Next.js 导出失败
  - 症状：静态导出报错或缺少页面。
  - 解决：检查 next.config.ts 的 output 与 distDir；确保所有页面可静态渲染。
- server.ts 打包失败
...
## 结论
本构建流程围绕“前端静态导出 + Node.js 服务器 + Docker/Nginx 托管”的方案展开。通过统一的构建脚本与明确的环境变量约定，能够稳定地产出可部署的静态站点与服务器产物。生产部署建议结合 CDN 与容器编排，进一步提升性能与可靠性。

[本节为总结性内容，不直接分析具体文件，故无“章节来源”]

## 附录
- 关键配置要点
  - 前端：next.config.ts 的 output/export、distDir；tsconfig.json 的 strict 与 moduleResolution；Dockerfile 的多阶段构建与 Nginx 配置。
