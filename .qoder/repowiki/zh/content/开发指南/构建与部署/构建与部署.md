...
## 更新摘要
**所做变更**
- 新增Next.js反向代理配置，将/api请求转发到后端服务，改进了API路由和部署能力
- 更新前端API调用方式，使用相对路径通过Nginx反向代理
- 完善Docker Compose中的环境变量配置，支持API前缀设置
- 优化前端开发体验，支持热重载和开发服务器
...
- [docker-compose.yml:100-101](file://docker-compose.yml#L100-L101)
...
MC["minio"]
ML["milvus"]
FE["frontend:5000<br/>NEXT_PUBLIC_API_URL=/api"]
BE["backend:8000"]
end
...
**更新**：前端服务端口映射为5000:5000，新增API前缀配置
...
**更新**：最新的版本解决了端口冲突问题，将前端服务端口从3000更改为5000，显著改善了开发环境的可用性。**新增**：通过Next.js反向代理配置，实现了更清晰的API路由和部署能力，前端代码使用相对路径`/api`，通过Nginx反向代理到后端服务。建议在生产环境中完善CORS策略、数据库迁移、日志与监控体系，并结合CI/CD实现自动化发布。
