...
## 核心组件
- 千问服务封装（QwenService）：负责模型调用、RAG增强对话、嵌入生成、工具调用增强
...
- 工具调用增强：支持Function Calling，内置6种教育数据查询工具，包括个人信息、成绩、课表、考试、学业进度、培养方案等
...
#### AI响应约束
系统在千问服务中引入了严格的AI响应约束，确保回答的准确性与合规性：
...
- **系统提示词强化**：
  - 明确AI助手的角色定位：广东财经大学校园AI助手
  - 强调数据准确性：严格基于提供的教务数据回答
  - 规范回答格式：直接给出答案，涉及数据时简要列出关键信息
  - 安全性要求：对于敏感信息绝不存储或泄露

#### 工具调用增强功能
系统实现了完整的Function Calling功能，支持6种教育数据查询工具：

- **工具定义**：
  - `query_personal_info`：查询个人信息（姓名、学号、专业、班级、学院）
  - `query_grades`：查询课程成绩（支持按课程名称或学期筛选）
  - `query_schedule`：查询课程表（支持按学期筛选）
  - `query_exam_schedule`：查询考试安排（包括考试时间、地点、座位号）
  - `query_academic_progress`：查询学业进度（已修学分、还需学分等）
  - `query_training_plan`：查询培养方案（课程体系结构、学分要求等）
  - `refresh_all_data`：重新爬取教务系统最新数据

- **工具执行流程**：
  1. 第一次调用：模型分析用户意图，决定是否需要调用工具
  2. 工具调用：执行相应的爬虫函数获取实时数据
  3. 结果整合：将工具结果作为上下文再次调用模型，获得最终回答
  4. 计费统计：合并两次调用的token用量

- **流式响应支持**：
  - 支持工具调用过程中的流式输出
  - 提供keep-alive机制防止连接超时
  - 支持思维过程（thinking）的逐步输出

**章节来源**
- [backend/app/services/qwen_service.py:31-165](file://backend/app/services/qwen_service.py#L31-L165)
- [backend/app/services/qwen_service.py:252-387](file://backend/app/services/qwen_service.py#L252-L387)
- [backend/app/api/chat.py:458-793](file://backend/app/api/chat.py#L458-L793)

### 流式响应与结构化直答

#### 流式响应机制
系统提供了完整的SSE（Server-Sent Events）流式响应支持：

- **流式接口**：`/api/chat/send-stream`
- **多阶段响应**：
  - 初始化阶段：发送conversation_id和trace_id
  - 工具调用阶段：显示"tool_call"状态，每2秒发送ping心跳
  - 模型流式阶段：逐步输出模型生成的内容
  - 结束阶段：发送done标记和最终的conversation_id

- **异常处理**：
  - 支持客户端断开连接的优雅处理
  - 提供错误兜底：无有效分片时自动降级为非流式
  - 详细的错误信息记录与上报
...
- **触发条件**：
  - 身份信息查询：当问题涉及"你是谁"、"我的信息"等身份相关问题
  - 地点查询：当问题涉及"哪里上课"、"校区"等地点相关信息
...
- **输出规范**：
  - 严格基于缓存数据回答，不补充未验证信息
  - 明确标注数据来源为"grounded_structured_data"
  - 提供必要的免责声明

**章节来源**
- [backend/app/api/chat.py:85-130](file://backend/app/api/chat.py#L85-L130)
- [backend/app/api/chat.py:541-756](file://backend/app/api/chat.py#L541-L756)