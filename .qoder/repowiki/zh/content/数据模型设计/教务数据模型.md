...
## 更新摘要
**变更内容**
- 新增EducationSyncSnapshot模型，增强数据审计能力
- 添加同步状态跟踪和数据版本管理
- 实现数据快照和批次同步机制
- 增强数据完整性验证和错误处理
...
系统采用前后端分离架构，后端使用Python FastAPI框架，数据库采用PostgreSQL，向量数据库使用Milvus，实现了传统教务数据与现代AI技术的有机结合。**最新更新**引入了教育数据快照和审计功能，通过`EducationSyncSnapshot`模型实现了完整的数据同步追踪和版本管理。
...
- [education_data.py:1-126](file://backend/app/models/education_data.py#L1-L126)
- [vector_store.py:1-164](file://backend/app/services/vector_store.py#L1-L164)
- [education_sync.py:1-185](file://backend/app/services/education_sync.py#L1-L185)
- [education_audit.py:1-54](file://backend/app/services/education_audit.py#L1-L54)
...
| user_id | Integer | 用户外键 | ForeignKey("users.id"), nullable=False, unique=True |
...
| last_updated | DateTime | 最后更新时间 | server_default=func.now(), onupdate=func.now() |
...
**新增** 教育同步快照模型用于记录每次数据同步的完整过程和状态，提供强大的审计和追踪能力。
...
|---------|---------|------|------|
| id | Integer | 主键ID | primary_key, index=True |
| user_id | Integer | 用户外键 | ForeignKey("users.id"), nullable=False, index=True |
| username | String | 学号冗余 | nullable=False, index=True, comment="学号冗余" |
| sync_key | String | 本次同步唯一键 | nullable=False, unique=True, index=True |
| schema_version | String | 数据契约版本 | nullable=False, default="v2" |
| status | String | pending/success/failed | nullable=False, default="pending" |
| sync_source | String | auto_login/manual/tool_refresh | nullable=False, default="auto_login" |
| is_active | Boolean | 是否当前激活快照 | nullable=False, default=False |
| crawl_success | Boolean | 爬取是否成功 | nullable=False, default=False |
| store_success | Boolean | 存储是否成功 | nullable=False, default=False |
| vector_success | Boolean | 向量化是否成功 | nullable=False, default=False |
| summary | JSON | 计数摘要 | default=dict |
| raw_payload | JSON | 原始爬取数据 | default=dict |
| normalized_payload | JSON | 标准化数据 | default=dict |
| error_message | Text | 失败信息 | nullable=True |
| created_at | DateTime | 创建时间 | server_default=func.now() |
| updated_at | DateTime | 更新时间 | onupdate=func.now() |

#### 快照状态管理

快照模型实现了完整的状态管理机制：

1. **同步状态** - pending → success → failed
2. **数据完整性** - 爬取、存储、向量化三阶段验证
3. **版本控制** - schema_version跟踪数据契约版本
4. **激活管理** - is_active标记当前有效快照

**章节来源**
- [education_data.py:50-70](file://backend/app/models/education_data.py#L50-L70)
...
| embedding | FLOAT_VECTOR | 向量 | dim=1536 |
...
系统采用分层架构设计，实现了业务逻辑、数据访问和外部服务的有效分离。**新增的快照机制**为整个系统提供了完整的审计和追踪能力。
...
Client->>API : 请求教务数据
API->>Service : 调用业务逻辑
Service->>Snapshot : 创建同步快照
Snapshot->>Model : 查询数据模型
Model->>DB : 执行数据库操作
DB-->>Model : 返回数据
Model-->>Service : 返回查询结果
Service->>Vector : 向量相似性搜索
Vector-->>Service : 返回相似文档
Service->>Snapshot : 更新快照状态
Snapshot->>DB : 写入快照数据
Service-->>API : 组装响应数据
API-->>Client : 返回最终结果
```
...
+String sync_source
+Boolean is_active
+Boolean crawl_success
+Boolean store_success
+Boolean vector_success
+JSON summary
+JSON raw_payload
...
EducationSyncSnapshot --> User : "一对一关系"
Grade --> User : "一对一关系"
Course --> User : "一对一关系"
```
...
**新增** 教育数据快照审计机制提供了完整的数据追踪和验证能力：

```mermaid
flowchart TD
Start([开始同步]) --> CreateSnapshot["创建快照记录"]
CreateSnapshot --> CrawlData["爬取原始数据"]
CrawlData --> ValidateData["验证数据有效性"]
ValidateData --> SuccessCheck{"数据有效?"}
SuccessCheck --> |否| MarkFailed["标记失败状态"]
SuccessCheck --> |是| NormalizeData["标准化数据"]
NormalizeData --> StoreData["存储到数据库"]
StoreData --> VectorizeData["向量化处理"]
VectorizeData --> ActivateSnapshot["激活快照"]
ActivateSnapshot --> CompleteSync["同步完成"]
MarkFailed --> ErrorLog["记录错误日志"]
ErrorLog --> End([结束])
CompleteSync --> End
```
...
**图表来源**
- [education_sync.py:18-185](file://backend/app/services/education_sync.py#L18-L185)
- [education_audit.py:18-53](file://backend/app/services/education_audit.py#L18-L53)
...
**最新更新**引入的教育数据快照和审计功能，显著增强了系统的可靠性和可维护性。通过`EducationSyncSnapshot`模型，系统实现了：

1. **完整的数据追踪** - 记录每次同步的详细过程和状态
2. **数据完整性验证** - 三阶段验证确保数据质量
3. **版本控制管理** - schema_version跟踪数据契约演进
4. **错误诊断能力** - 详细的错误信息和日志记录
5. **激活快照管理** - 自动管理当前有效数据版本
